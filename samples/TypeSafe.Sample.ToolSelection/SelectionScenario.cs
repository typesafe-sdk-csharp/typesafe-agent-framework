...
namespace TypeSafe.Sample.ToolSelection;
...
public static partial class SelectionScenario
{
    ...
    public static TypeSafeToolSelectionOptions CreateOptions() => new()
    {
        TopK = 1,
    ...
    public static TypeSafeToolSelectionProvider CreateProvider(ITypeSafeClient client) => new(client, CreateOptions());

    public static IList<AITool> CreateCatalog() => [.. new OfflineGitHubCatalog().Tools];

    public static string CreateReviewPrompt(GitHubPullRequestTarget target) =>
        $"Review pull request #{target.Number} in {target.Repository}. Use the available read-only GitHub tool and " +
        "provide a concise review of the title, status, changed areas, and notable risks. Do not modify GitHub resources.";
